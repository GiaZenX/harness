...
- **The ladder is BUILT, not prose (DEC-0077, DEC-0078; the rules are DEC-0034's, the endpoints
  ...
  **opus** and so do planning, design and QA — `DEC-0095` (1)/(2); the ARCHITECTURE starts on the
  ...
  `create-task --rung sonnet` — an expected output that is a test module (`test_x.py`,
  `x_test.go`) or lies in a `test`/`tests` tray, or a criterion SENTENCE naming a test as a
  runner or with its verdict (»der Test wird rot«). A document merely named after tests
  (`docs/test-plan.md`) is not one, and a sentence that denies one (»kein Test nötig«) is not
  an acceptance: both keep the default, and the lease's `why` says which of the two it was
  (`DEC-0097` (2); the reader is `kernel.dispatch.acceptance_is_test_shaped`). **What you do
...
- **A QA FAIL and the `escalation: true` flag of §14a:** the flag stays and is still yours to set on
  ...
  declared threshold of failed runs is reached (`DEC-0096`). A run QA classifies as `mechanical` does not count
  (`DEC-0107`): only the verifying role can write that field — its command is in that role's
  skill, and `gate_dispatch` refuses every other writer, YOU included. Silently ignoring
