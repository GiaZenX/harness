...
def main():
    ...
    # require a passing QA/validation/acceptance report — bound to THIS PRD when it is known.
    ...
    # binding at all (indirect task->PRD linkage) does the file-level check apply. No PyYAML ->
    # file-level fallback (fail-open philosophy).
    passing = False
    ...
    if not passing:
        block("no passing QA/validation report for %s in project_memory — run the QA gate "
              "(a passing review/test/acceptance report for this PRD) before merge/push."
              % (target or "this work"))
