...
- **The ladder is BUILT, not prose (DEC-0077, DEC-0078; the rules are DEC-0034's, the endpoints
  ...
  **opus** and so do planning and the reviewer — `DEC-0095` (1)/(2); the METHOD DESIGN, this kit's
  ...
  the ADDRESS of a test -- a test module path, with or without the node a runner appends after
  a double colon -- in an expected output or in a referenced criterion. A DESCRIPTION buys nothing since `DEC-0112`: a promise (»der Test
  wird rot«), a denial (»kein Test nötig«) and a document merely named after tests
  (`docs/test-plan.md`) all keep the default, because four rounds found four holes in the reader
  that judged such sentences and every hole granted the cheap rung. The lease's `why` names the
  shaped form (`DEC-0097` (2), `DEC-0112`; the reader is
  ...
  **What you do with it:** when the header's `rung` is not the role's own pin,
  ...
  escalation ladder any more. On Codex no hook sees a spawn (`H173`): your row under `by_provider` is an instruction you
