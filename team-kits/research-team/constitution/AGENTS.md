...
- **A validation FAIL and the `escalation: true` flag of §14a:** the flag stays and is still yours to
  ...
  kit's declared threshold of failed runs is reached (`DEC-0096`). A run the reviewer classifies as `mechanical` does
  not count (`DEC-0107`): only the verifying role can write that field — its command is in that
  role's skill, and `gate_dispatch` refuses every other writer, YOU included. Silently ignoring
